...
def main():
    ...
    # import pickle
    # with open('../data/dataprocessed.pkl','wb') as f:
    #     pickle.dump([data_idx,data_train_idx,data_test_idx],f)
#     with open('../data/data_feature.pkl','wb') as f:
#         pickle.dump([mutation_feature,drug_feature,gexpr_feature,methylation_feature],f)
            
    # alldata = {'mutation_feature':mutation_feature,'drug_feature':drug_feature,'gexpr_feature':gexpr_feature,'methylation_feature':methylation_feature,'data_idx':data_idx}
    ...
    if args.evaluate:
        ...
        model.load_weights('../checkpoint/MyBestDeepCDR_%s.h5'%model_suffix)
        Y_pred,Y_test = ModelEvaluate(model,X_drug_data_test,X_mutation_data_test,X_gexpr_data_test,X_methylation_data_test,Y_test,cancer_type_test_list,'%s/DeepCDR_%s.log'%(DPATH,model_suffix))
        np.savez('%s/DeepCDR_%s.npz'%(DPATH,model_suffix),pred = Y_pred,test = Y_test)
